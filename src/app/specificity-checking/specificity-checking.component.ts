...
export class SpecificityCheckingComponent implements OnInit {
  ...
  ngOnInit() {
    this.serverService.setConfig().subscribe(data => {
      this.serverService.getSupportedGenomes().subscribe(data => {
        this.serverService.supportedGenomes = data;
  ...
  reset(){
    this.specForm = this.fb.group({
      GENOME: [this.dataService.main.task.spec_check_data.GENOME, this.pvService.specValidator('GENOME')],
      TOTAL_SPECIFICITY_MISMATCH: [this.dataService.main.task.spec_check_data.TOTAL_SPECIFICITY_MISMATCH, this.pvService.specValidator('TOTAL_SPECIFICITY_MISMATCH')],
      SEND_SPECIFICITY_MISMATCH: [this.dataService.main.task.spec_check_data.SEND_SPECIFICITY_MISMATCH, this.pvService.specValidator('SEND_SPECIFICITY_MISMATCH')],
